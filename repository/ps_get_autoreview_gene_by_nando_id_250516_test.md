...
WHERE { 
  ...
  BIND(IF(CONTAINS(STR(?source_url), "orphadata"), ?exactMatch_disease,
          IF(CONTAINS(STR(?source_url), "mim2gene_medgen"), ?exactMatch_disease, ?source_url)) AS ?gencc_url)
  ...
  BIND(IF(CONTAINS(STR(?source_url), "orphadata") || CONTAINS(STR(?source_url), "mim2gene_medgen"),
